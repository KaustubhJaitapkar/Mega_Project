...
export default function ParticipantProfileClient({ user }: Props) {
  ...
  const [form, setForm] = useState({
    name: user.name,
    college: user.profile?.company || '',
    yearOfStudy: user.profile?.experience || '',
    ...
    sponsorVisible: user.profile?.isPublic ?? true,
    isLookingForTeam: user.profile?.isLookingForTeam ?? false,
  });
      ...
      <form onSubmit={handleSave}>
          ...
          {filtered.length > 0 && skillInput && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem', marginBottom: '0.6rem' }}>
              {filtered.slice(0, 6).map((s) => (
                <button key={s} type="button" onClick={() => addSkill(s)} style={{ padding: '0.2rem 0.5rem', background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-sm)', color: 'var(--text-secondary)', fontSize: '0.72rem', cursor: 'pointer' }}>{s}</button>
              ))}
            </div>
          )}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
            {form.skills.map((skill) => (
              <span key={skill} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.25rem 0.55rem', background: 'var(--accent-dim)', border: '1px solid rgba(232,164,74,0.2)', borderRadius: 'var(--radius-sm)', color: 'var(--accent)', fontSize: '0.75rem', fontWeight: 500 }}>
                {skill}
                <button type="button" onClick={() => setForm((p) => ({ ...p, skills: p.skills.filter((s) => s !== skill) }))} style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', fontSize: '0.85rem', padding: 0, lineHeight: 1 }}>&times;</button>
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="org-section" style={{ marginBottom: '0.75rem' }}>
          ...
          <div style={{ marginTop: '0.75rem' }}>
            <FileUpload value={form.resumeUrl} onChange={(val) => setForm({ ...form, resumeUrl: val })} label="Resume / CV" accept=".pdf,.doc,.docx" hint="Upload your resume (PDF, DOC, DOCX) - Max 10MB" maxSizeMB={10} />
          </div>
        </div>
