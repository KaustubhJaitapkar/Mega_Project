...
export default function MyTeamClient({
  ...
  initialHackathonId,
  inviteRequestId,
  registrationStatus: initialRegStatus,
  teamsForHackathon: initialTeams,
}: MyTeamClientProps) {
  const router = useRouter();
  const [hackathons, setHackathons] = useState<HackathonOption[]>(initialHackathons);
  ...
  const [myTeam, setMyTeam] = useState<Team | null>(
    initialTeams.find((t) => t.members.some((m) => m.user.id === userId)) || null
  );
  const [incoming, setIncoming] = useState<JoinRequest[]>([]);
  const [outgoing, setOutgoing] = useState<JoinRequest[]>([]);
  const [invites, setInvites] = useState<any[]>([]);
  const [mySkills, setMySkills] = useState<string[]>(userSkills);
  ...
  const [isRegistered, setIsRegistered] = useState(initialRegStatus[initialHackathonId] || false);
  ...
  const loadTeams = useCallback(async () => {
    ...
    setError('');
    try {
      const mine = teams.find((t: Team) => t.members.some((m) => m.user.id === myUserId)) || null;
      setMyTeam(mine);
